    ...
    def train(self, ):
        ...
        if self.dev == "cpu":
            ...
            data_loader = DataLoader(dataset, batch_size=self.batchSize,
                ...
                sampler=torch.utils.data.RandomSampler(dataset, replacement=True, num_samples=(self.totalSteps-num_steps)*self.batchSize),
        ...
        for step, data in enumerate(tqdm(data_loader, initial=num_steps, total=self.totalSteps)):
            ...
            batch_x_0 = batch_x_0.to(self.device)
            batch_class = batch_class.to(self.device)
            ...
            with torch.no_grad():
                if self.dev == "cpu":
                    vae_output = self.model.VAE.encode(batch_x_0).latent_dist
                else:
                    vae_output = self.model.module.VAE.encode(batch_x_0).latent_dist

                # Get sample from latent distribution using the reparameterization trick
                # batch_x_0 = vae_output.mean
                batch_x_0 = vae_output.sample()
