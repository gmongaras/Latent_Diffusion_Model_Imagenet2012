...
def infer(
    ...
    # Load in the imagenet_class_to_string file
    with open("imagenet_class_to_string.pkl", "rb") as f:
        class_to_string = pickle.load(f)

    # If class_label is -1, then sample a random class
    if class_label == -1:
        class_label = random.randint(0, num_classes-1)
        print(f"Random class selected: {class_label}, {class_to_string[str(class_label)]}")
